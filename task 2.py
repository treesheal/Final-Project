...
def get_data():
    ...
    while len(first_name) < 1 or not str.isalpha(first_name):
        first_name = raw_input("First name is required, please re-enter.")

    #get their last name
    last_name = raw_input("Please enter last name: ")
    while len(last_name) < 1 or not str.isalpha(last_name):
        last_name = raw_input("Last name is required, please re-enter.")
...
def choice_two():
    ...
    print new_user + 'has been added to our system. Select Existing User during your future interactions.'



#creating def for selection 3
def choice_three():
    print "Welcome! Please create a new User to use this kiosk." "\n"
